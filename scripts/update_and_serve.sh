...
cd /app
ORIGINAL_DATA_DIR="/app/public/data"
TEMP_DATA_DIR="$TEMP_DIR/public/data"
...
DATA_DIR="$TEMP_DATA_DIR" /usr/local/bin/pnpm update-feeds >> "$LOG_FILE" 2>&1
if [ $? -ne 0 ]; then
...
echo "Updating public data directory..." >> "$LOG_FILE"
rm -rf "$ORIGINAL_DATA_DIR"
mkdir -p "$ORIGINAL_DATA_DIR"
cp -r "$TEMP_DATA_DIR"/. "$ORIGINAL_DATA_DIR"/
echo "Public data directory updated." >> "$LOG_FILE"

# Build the project
echo "Building the project..." >> "$LOG_FILE"
cd /app
/usr/local/bin/pnpm build >> "$LOG_FILE" 2>&1
if [ $? -ne 0 ]; then
